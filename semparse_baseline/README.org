...
** Training with post-processing answers and limiting train set
#+begin_src sh
python -m $MODULE.train --input_dir $PROCESSED_PATH --output_dir $OUTPUT_DIR_PATH --save_dir $TRAIN_LOG_PATH --model_name_or_path "$MODEL_NAME_OR_PATH" --postprocessing-answer --train-set-percent 100
#+end_src
