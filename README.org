...
python -m domain.kqapro.run --config $TEST_CONFIG --model-checkpoint-dir $CHECKPOINT_PATH --test-dir $TEST_DIR_PATH --additional-config config.additional.no_candidate --no-arg-candidate-for $ACTION_NAME
#+end_src
