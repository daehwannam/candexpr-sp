...
python -m domain.kqapro.run --config config.test_on_val_set --model-checkpoint-dir $dir --test-dir-name $dir/test-on-val-set --additional-config config.additional.using_arg_candidate=False

dir="$BASE_DIR/no-constrained-decoding:best"
python -m domain.kqapro.run --config config.test_on_val_set --model-checkpoint-dir $dir --test-dir-name $dir/test-on-val-set --additional-config config.additional.constrained_decoding=False
