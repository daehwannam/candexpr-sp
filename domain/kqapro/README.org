...
#+begin_src sh
python -m domain.kqapro.run --config config.kqapro.train_default
#+end_src

*** Training for multiple decoding strategies
#+begin_src sh
python -m domain.kqapro.run --config config.kqapro.train_for_multiple_decoding_strategies
#+end_src

*** Training for multiple decoding strategies with a portion of the train set
#+begin_src sh
PERCENT=5  # between 0 to 100
python -m domain.kqapro.run --config config.kqapro.train_for_multiple_decoding_strategies --extra-config config.kqapro.extra.train_set_portion --train-set-percent $PERCENT
#+end_src

*** Training for multiple decoding strategies with a portion of the train set where inferencing_subtypes=False
#+begin_src sh
PERCENT=5  # between 0 to 100
python -m domain.kqapro.run --config config.kqapro.train_for_multiple_decoding_strategies --extra-config config.kqapro.extra.train_set_portion_no_inferencing_subtypes --train-set-percent $PERCENT
#+end_src

*** Training for multiple decoding strategies with batch_size=128
#+begin_src sh
python -m domain.kqapro.run --config config.kqapro.train_for_multiple_decoding_strategies --extra-config config.kqapro.batch.size=128
#+end_src
...
python -m domain.kqapro.run --config $TEST_CONFIG --model-learning-dir $MODEL_LEARNING_DIR_PATH
#+end_src
...
python -m domain.kqapro.run --config $TEST_CONFIG --model-learning-dir $MODEL_LEARNING_DIR_PATH --model-dir-name $MODEL_DIR_NAME
#+end_src
...
TEST_CONFIG='config.kqapro.test_on_val_set'  # or 'config.kqapro.test_on_test_set'
MODEL_PATH='path/to/model'
COMMON_TEST_CMD="python -m domain.kqapro.run --config config.kqapro.$TEST_CONFIG --model-path $MODEL_PATH"
...
TEST_DIR_PATH='path/to/test-result:no-ac-no-dut'
...
python -m domain.kqapro.run --config $TEST_CONFIG --model-path $MODEL_PATH --test-dir $TEST_DIR_PATH --extra-config config.kqapro.extra.no_arg_candidate --no-arg-candidate-for $ACTION_NAME
#+end_src
